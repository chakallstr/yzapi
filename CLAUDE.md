...
│   ├── db/{schema.ts,client.ts,migrations/}  — Drizzle tek-kaynak + sequential SQL (0000…0024, 25 dosya)
...
- **`provider_profiles`** (`id, baseUrl, apiKeyCipher, enabled, supportedModelIds, modelMap`) seed: `scripts/seed-provider-profiles.ts` (sunucuda ayrı koşar, deploy çalıştırmaz; `WELLFLOW_API_KEY`/`POPUSK_API_KEY` env → DB cipher). `active_provider_id` artık yalnız FALLBACK (+ web-search + Gözcü `models_reachability` probe + connection-test). `AI_PROVIDER_*` env yalnız bootstrap.
...
- **Tablolar** (`0019`/`0020`/`0022`): `packages` (günlük istek limiti, süre_gün, `allowed_models` whitelist jsonb, fiyat TL/USD, enabled), `user_package_entitlements` (kullanıcı başına aktif hak + günlük sayaç/reset), `redeem_codes` + `redeem_code_uses` (promo/hediye kod; tip=bakiye│paket, `max_uses`/`per_user_once`/`expires_at`), `account_delivery_orders` (hesap-teslim siparişi; durum `bekliyor→teslim_edildi`).
- **`entitlement-service.ts`:** `checkPackageCoverage()` (model whitelist'te + kota dolmamış mı — salt-read), `tryReservePackageSlot()` (atomik UPDATE + günlük reset), `releasePackageSlot()`, `recordPackageUsage()` → `usage_records` satırı `billed_via='package'`, **cost=0** (paradan düşmez, kotadan düşer). `usage_records`'a `billed_via`+`entitlement_id` 0019'da eklendi.
- **Route/UI:** `routes/packages.ts` → `/api`'ye mount (public katalog + purchase `Idempotency-Key` + `/redeem`); frontend `tab-packages.jsx` (paket grid + redeem input + aktif hak listesi + `account_delivery` için iletişim sorma).
...
| Migration | 25 dosya `0000…0024` (highest `0024_provider_fallback.sql`; `0017_gozcu.sql`/`0018_added_model_context.sql` var). Faz eklemeleri: `0019_packages` · `0020_redeem_codes` · `0021_added_model_image` (added_models'a `type`+`image_price_usd`) · `0022_github_oauth_account_delivery` · `0023_user_lang` · `0024_provider_fallback` (`provider_profiles.fallback_provider_id`). Yeni tablo: `... IF NOT EXISTS` + `schema.ts` Drizzle + `meta/_journal.json` sıralı güncellenir; deploy `db:migrate` çalıştırır |
