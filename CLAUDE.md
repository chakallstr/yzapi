...
| Migration | 26 dosya `0000…0025` (highest `0025_user_role.sql`). Önemli: `0017_gozcu` · `0019_packages` · `0020_redeem_codes` · `0021_added_model_image` (type+image_price_usd) · `0022_github_oauth_account_delivery` · `0023_user_lang` · `0024_provider_fallback` (`provider_profiles.fallback_provider_id`) · `0025_user_role` (partner RBAC). ⚠️ İzole deploy için migration numarasını canlı sıraya göre ver. Yeni tablo: `IF NOT EXISTS` + `schema.ts` Drizzle + `meta/_journal.json` sıralı; deploy `db:migrate` koşar |
