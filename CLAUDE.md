...
- **⚠️ Canlı paketler (2026-06-09):** Tüm paketler (`codex-api-chatgpt`, `deneme-25-istek`, `rika-gpt-deneme`) + tüm `user_package_entitlements` + `redeem_codes` **SİLİNDİ** (Ufuk kararı). Altyapı (tablolar/kod) hazır; yeni paketler `scripts/seed-*.ts` + `db:seed` ile eklenecek.
...
- **Canlı durum (2026-06-09):** VPS commit `b5a6c72` (sync-20260609T050143Z). Aktif özellikler: token-billing, i18n TR/EN, partner RBAC, görsel API, provider failover+circuit-breaker. **Paketler yok** (tüm paket+entitlement+redeem silindi, altyapı hazır). **Provider:** wellflow=Claude, closerouter=GPT/Gemini/o-series, rika=disabled. Maintenance notice özelliği lokal+QA tamam ama canlıda DEĞİL (Ufuk "beklet" dedi).
