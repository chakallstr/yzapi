...
- **Canlı provider topolojisi (2026-06-09):** `rika` → **DISABLED** (ai.rika.wtf; gpt-5.5/gpt-5.4 closerouter'a taşındı); `wellflow` (api.wellflow.dev/v1) → **yalnız Claude** (opus-4.8, opus-4-7, opus-4-6, sonnet-4-6, haiku-4-5); `closerouter/Popusk` (api.claude-popusk.shop/v1) → GPT ailesi (gpt-5.5/gpt-5.4 dahil), o-serisi, Gemini (Claude **YOK**). Metro silindi. `system_api_config.active_provider_id=closerouter` (fallback/probe). `default_context_limit_tokens=128000`, `default_max_tokens_per_request=128000`, `default_stream_timeout_ms=300000`.
...
- **Canlı paketler (2026-06-11):** `gpt-gunluk-1000` (150₺/1g) · `gpt-haftalik-1000` (1000₺/7g) · `gpt-aylik-1000` (3500₺/30g) — hepsi 1000 istek/gün, `["gpt-5.5","gpt-5.4"]`, **95k bağlam limiti (SESSİZ — Ufuk kararı: aciklama/panelde gösterilmez, yalnız aşımda 400 hatasında görünür)**, **satista=false** ("en kısa sürede satışta", satın alınamaz), upstream override alanları **BOŞ** (Ufuk ayrı endpoint+key verecek, admin "API" butonuyla doldurulacak). 2026-06-09'daki eski paketler silinmişti; bunlar yeni set.
...
| Migration | 33 dosya `0000…0032` (highest `0032_package_provider_override.sql`). Önemli (numaralar DOSYA ADLARIYLA doğrulandı 2026-06-11): `0017_gozcu` · `0019_provider_fallback` (`provider_profiles.fallback_provider_id`) · `0020_packages` · `0021_redeem_codes` · `0022_added_model_image` · `0023_github_oauth_account_delivery` · `0024_user_lang` · `0025_user_role` (partner RBAC) · `0026_configurable_packages` · `0027_package_context_limit` · `0028_usage_provider_profile` (`usage_records.provider_profile_id` text) · `0029_package_tpm_limit` · `0030_package_satista` · `0031_min_topup_5usd` (`min_bakiye_tl`→225) · `0032_package_provider_override`. ⚠️ İzole deploy için migration numarasını canlı sıraya göre ver. Yeni tablo: `IF NOT EXISTS` + `schema.ts` Drizzle + `meta/_journal.json` sıralı (`when` artan); deploy `db:migrate` koşar |
...
- **Canlı durum (2026-06-11):** VPS commit `28ebd1a` (sync-20260610T220115Z). Aktif özellikler: token-billing, i18n TR/EN, partner RBAC, görsel API, provider failover+circuit-breaker, **3 GPT istek paketi (satista=false, "yakında satışta")**, **min yükleme $5**, paket upstream override (alanlar BOŞ). **Provider:** wellflow=Claude, closerouter=GPT/Gemini/o-series, rika=disabled. Maintenance notice özelliği lokal+QA tamam ama canlıda DEĞİL (Ufuk "beklet" dedi).
