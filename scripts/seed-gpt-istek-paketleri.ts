...
const PAKETLER = [
  {
    ...
    fiyatTL: 150,
    aciklama:
      "GPT 5.5 ve GPT 5.4 modellerinde günde 1.000 istek hakkı — 1 gün boyunca. Her istek 1 sayılır.",
    displayOrder: 1,
  },
  {
    ...
    fiyatTL: 1000,
    aciklama:
      "GPT 5.5 ve GPT 5.4 modellerinde günde 1.000 istek hakkı — 7 gün boyunca. Her istek 1 sayılır.",
    displayOrder: 2,
  },
  {
    id: "gpt-aylik-1000",
    ...
    fiyatTL: 3500,
    aciklama:
      "GPT 5.5 ve GPT 5.4 modellerinde günde 1.000 istek hakkı — 30 gün boyunca. Her istek 1 sayılır.",
    displayOrder: 3,
  },
];
  ...
  for (const p of PAKETLER) {
    await dbSql`
      ...
      ON CONFLICT (id) DO UPDATE SET
        ...
        updated_at          = now()
    `;
    console.log(`✓ ${p.id} eklendi/güncellendi (${p.fiyatTL} TL, ${p.sureGun} gün, 1000 istek/gün, 95k sessiz bağlam limiti, satista=false).`);
  }
  await dbSql.end();
}
