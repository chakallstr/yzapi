    ...
    try {
      if (streaming) {
        ...
        if (!resp.ok || !resp.body) {
          ...
          throw new Error(e?.error || `Hata (${resp.status})`);
        }
        ...
        const content = j?.choices?.[0]?.message?.content || '(boş yanıt)';
        setMessages((prev) => {
          const c = [...prev];
      ...
      setError(e.message || 'Sohbet hatası');
      setMessages((prev) => {
        const c = [...prev];
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <Card pad={12}>
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <button onClick={exportChat} disabled={!messages.length}>Dışa aktar</button>
        </div>
      </Card>

      <Card pad={12}>
        <div ref={scrollRef} style={{ maxHeight: 440, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 8 }}>
          {messages.length === 0 && <div style={{ color: 'var(--ink-2)' }}>Mesaj yok. Bir şeyler yazın.</div>}
          {messages.map((m, i) => (
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
          <textarea
            ...
            placeholder="Mesaj… (Enter gönder, Shift+Enter satır)"
            rows={2}
            style={{ flex: 1 }}
          />
          <button onClick={send} disabled={busy || !model}>{busy ? '…' : 'Gönder'}</button>
        </div>
      </Card>
    </div>
  );
