...
Examples of behaviors to watch for — these are illustrative, not exhaustive:
...
Use browser MCP to enumerate all assets on the page (images with src/alt/dimensions/parent-info, videos, background images, svg count, fonts, favicons). Then write a download script that fetches everything to `public/`. Use batched parallel downloads (4 at a time) with proper error handling.
...
### Step 1: Extract
For each section, use browser MCP to extract everything: screenshot the section in isolation; extract CSS for every element (full computed-style walk, depth ≤4); extract multi-state styles (capture both states, diff them); extract real content (textContent, alt, aria, placeholder; click each tab for per-state content); identify assets (downloaded images/videos, icon components, layered images); assess complexity (how many distinct sub-components).

### Step 2: Write the Component Spec File
For each section (or sub-component), create a spec file in `docs/research/components/<component-name>.spec.md` with: Overview (target file, screenshot, interaction model), DOM Structure, Computed Styles (exact values per element), States & Behaviors (trigger, state A/B, transition, implementation approach), Per-State Content, Assets, Text Content (verbatim), Responsive Behavior (desktop/tablet/mobile + breakpoint). Fill every section or mark N/A.

### Step 3: Dispatch Builders
Based on complexity, dispatch builder agent(s) in worktree(s). Simple section (1-2 sub-components): one builder. Complex section (3+ distinct sub-components): one agent per sub-component plus one for the wrapper. Each builder receives the full spec file contents inline, the section screenshot path, which shared components to import, the target file path, instruction to verify `npx tsc --noEmit`, and responsive breakpoint values. Don't wait — dispatch then move to extracting the next section.

### Step 4: Merge
As builder agents complete: merge their worktree branches into main, resolve conflicts intelligently, verify `npm run build` after each merge, fix type errors immediately.

## Phase 4: Page Assembly
After all sections are built and merged, wire everything together in `src/app/page.tsx`: import all section components, implement page-level layout (scroll containers, columns, sticky, z-index), connect real content to props, implement page-level behaviors (scroll snap, scroll-driven animations, theme transitions, intersection observers, smooth scroll), verify `npm run build`.

## Phase 5: Visual QA Diff
After assembly, do NOT declare complete. Take side-by-side comparison screenshots, compare section by section at desktop (1440px) and mobile (390px). For each discrepancy: check the spec file (was the value extracted correctly?), re-extract/fix as needed. Test all interactive behaviors. Only after this visual QA pass is the clone complete.

## Pre-Dispatch Checklist
Before dispatching ANY builder agent, verify: spec file written with ALL sections filled; every CSS value from getComputedStyle (not estimated); interaction model identified; every state's content/styles captured; scroll-driven trigger/before-after/transition recorded; hover before/after/timing recorded; all images identified (including overlays); responsive behavior documented; text verbatim; builder prompt under ~150 lines.

## What NOT to Do
- Don't build click-based tabs when the original is scroll-driven (or vice versa). Determine interaction model FIRST.
...
## Completion
When done, report: total sections built, total components created, total spec files written, total assets downloaded, build status, visual QA results, any known gaps or limitations.
