...
- **Date:** 2026-06-05
- **Status:** Approved (design); implementation pending. **No deploy in design session.**
...
- Migration `0019_provider_fallback.sql`: `ALTER TABLE provider_profiles ADD COLUMN IF NOT EXISTS fallback_provider_id text;` (nullable, **soft ref — sert FK yok**).
- `schema.ts`: `fallbackProviderId: text("fallback_provider_id")`. `meta/_journal.json` sıralı güncelleme.
...
- `fallback`: primary **pinli profilden** geldiyse (profileId≠null) ve o profilin `fallbackProviderId`'si set ise → o profili enabled+anahtarlı profiller arasında çöz; fallback ctx = onun `baseUrl/apiKey/modelMap`'i.
- **Kritik kurallar:**
...
- **Streaming commit point:** `forwardChatStream` / `forwardChatStreamAsResponses` / streaming `/messages` refactor: (a) upstream `fetch` budget-signal ile, (b) istemciye **yazmadan önce** `res.status` incele, (c) status ∈ {502,503,504} → pre-commit `UpstreamUnavailableError` fırlat, (d) yalnız status uygunsa pipe başlat (= commit). Başarı/no-fallback yolunda mevcut SSE davranışı **birebir korunur** (responses ikizi parite).
- Non-streaming (chat/text/responses JSON): tam upstream cevabı alınır; eligible status/throw → failover; aksi → return.
...
- **Integration (gerçek PG + nock upstream):** 503→fallback servis (200); 400→failover yok; connect-timeout(bütçe)→failover; streaming pre-byte 503→failover, mid-stream drop→failover YOK; **billing: tek reserve+tek settle, çift tahsil yok**, ikisi de patlarsa 0 tahsil+tam iade; breaker 3 hata→primary atlanır (upstream primary'ye gitmez), cooldown→half-open→recover.
